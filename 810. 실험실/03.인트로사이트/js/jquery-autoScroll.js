...
/* 
    ...
    -> 함수명은 this 키워드로 등록
    예) function my(){...} -> this.my = () => {}

    2. 변수는 외부에 공개할 것만 this 키워드로 등록
    -> 일반적으로 let, const를 모두 변경할 필요는 없고 내가 인스턴스 생성시 접근해야 할 변수만 this로 등록함

    3. 할당형 변수는 호출 호이스팅이 불가하므로 상단에서 바로 함수를 호출하거나 이벤트 등록한 경우, 이를 생성자 함수 하단으로 이동시킴
 */
