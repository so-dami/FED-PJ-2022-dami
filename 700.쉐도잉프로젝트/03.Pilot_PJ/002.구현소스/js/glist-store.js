...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        setData(dt,pm){ // pm - 배열데이터 순번
            ...
            // 2. 로컬스토리지 객체데이터 가져오기
            // 입력된 데이터는 문자형 객체이므로 다시 파싱하여 원래 객체로 복원함
            ...
            // 4. 객체형을 문자형으로 변환 후 로컬스토리지에 반영
            localStorage.setItem("cart", JSON.stringify(org));
            console.log("반영 후 로칼:", localStorage.getItem("cart"));

            // 5. 카트 애니메이션 버튼을 등장시켜 카트리스트까지
        ...
        cartAni(dt,pm){
            ...
            console.log("카트 애니",pm);

            // 0. 생성될 카트 이미지 지우고 시작(하나만 생성)
            $("#mycart").remove();
