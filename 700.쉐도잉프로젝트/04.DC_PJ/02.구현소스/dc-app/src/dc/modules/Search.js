...
// 왜? 배열의 정렬 정보가 컴포넌트에 포함될 경우
// 정렬 변경이 되지 않는다! 따라서 컴포넌트 바깥 위에서
// 데이터 정렬이 변경된 원본 배열을 만들어준 것을
// 컴포넌트 내부에서 사용하도록 하면 기존 정렬 변경이 반영됨!
cat_data.sort((x,y)=>{
    return x.cname===y.cname?0:x.cname>y.cname?1:-1;
})

function Search(props) { // props.skw - 전달 키워드
