...
/*******************************************************************
    ...
    3. 설치
        ...
        -> 설치 후 import로 사용할 페이지에 패키지를 포함시킴

        import Vue from 'vue'
        impor VueRouter from 'vue-router'
        Vue.use(VueRouter) -> 뷰JS에서 라우터 사용 등록 필수

        3) 뷰 인스턴스 객체 내부에 옵션으로 반드시 등록해야 사용 가능함
            예) new Vue({
                    el: "#app",
                    router, -> 등록 필수
                })
            ...
            ※ 참고 ※
            router 변수와 옵션 routes 변수는 변경 불가
            이미 라이브러리에서 변수명을 제한하고 있음

    4. 버전: Vue 2.x 버전일 경우 Vue Router 3.x 버전을 사용함
    5. 내장 컴포넌트
        1) <router-view>: 라우트/루트(route)와 일치하는 컴포넌트를 랜더링함
        2) <router-link>: 라우트/루트(route) 링크를 생성함
...
Vue.component("main-area", {
    ...
    // 뷰엑스 스토어의 변수 store를 전역 표시 $store 라고
    // 표기해야 유효함(에러 없음)
    // 더 정확한 문법: this.$store 라고 써야 하지만
    // 이 파일이 일반 JS에서 실행되므로 변수할당된
    // 뷰JS 인스턴스가 영역 안에서 실행되므로
    // 뷰인스턴스 자신인 this를 쓰지 않아도 자동으로 this로 처리됨
    template: `
        <main>
